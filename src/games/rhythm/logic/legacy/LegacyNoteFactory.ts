...
export class LegacyNoteFactory {
    public static createLegacyNotes(
        ...
        if (measureMap.size === 0) {
            const trackConfig = MelodyAnalyzer.suggestGapFilling(midi);
            autoGeneratedConfig = new Map();
            trackConfig.forEach((tIdx, mIdx) => {
                const track = midi.tracks[tIdx];
                if (track) {
                    autoGeneratedConfig!.set(mIdx, track.channel);
                }
            });
        }
        ...
        // RESTORE: Legacy BPM-relative hold threshold (Sanctuary Restoration)
