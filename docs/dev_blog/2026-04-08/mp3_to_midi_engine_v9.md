...
리듬 게임에서 MP3 음원을 재생할 때 **채보(노트)가 전혀 나오지 않는 문제**를 최종적으로 해결하고,
음원의 실제 드럼/보컬 리듬을 정확하게 추출하는 고품질 변환 엔진을 완성한다.

---

## 1. 오늘 발견한 버그 목록

### 1-1. [긴급] 스테일 JSON 비트맵이 MIDI 데이터를 덮어쓰는 문제
...
| そばにいるから | 89.1 | 606 | 503 | librosa HPSS + onset_detect |
| 想いは廻る | 143.6 | 763 | 736 | librosa HPSS + onset_detect |
